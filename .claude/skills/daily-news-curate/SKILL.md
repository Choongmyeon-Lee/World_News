...
- 입력: 시간창(직전 24시간 KST), 카테고리 4개, 어제 보고서 경로(있으면)
...
- 입력: `_workspace/02_verified.json` (HIGH/MEDIUM 사건만 분석)
...
- 입력: `_workspace/02_verified.json`, `_workspace/03_analyzed.json`, 어제 보고서(있으면), 템플릿(`assets/report-template.md`)
- 산출물: `reports/{오늘 KST 날짜}.md`
- 완료 신호: 파일 경로 + 분석 메타(HIGH/MEDIUM/LOW/DROP 건수)
