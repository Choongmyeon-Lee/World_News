...
## ⚠️ Anti-hallucination 가드레일 (위반 시 작업 무효)

본 작업은 매일 변하는 시장·정세 정보를 다룬다. **당신의 학습 데이터(training cutoff)는 작업 기준 시점보다 1년+ 오래됐을 가능성이 매우 높다.** 학습 데이터의 어떤 수치·사건·URL도 "오늘의 사실"로 간주하지 말 것.

**불변 규칙:**
1. **모든 수치는 WebSearch/WebFetch 결과의 1차 출처 본문에서만** 인용 가능. 학습 데이터의 익숙한 패턴(예: NASDAQ ~17,5XX, S&P ~5,8XX, USD/KRW ~1,350)이 떠오르면 그 자체가 hallucination 신호 — **반드시 검색으로 재확인**.
2. **출처 URL은 검색 결과에 실제로 등장한 URL만 사용.** 절대 URL을 fabricate하지 말 것. "있을 법한" Reuters/Bloomberg URL을 만들어 적는 것은 사실 위조이며 작업 무효 사유.
3. 모든 URL은 `WebFetch`로 200 OK + 본문 fetch 성공 + 본문에 해당 사실이 실제 등장하는지 확인 후에만 `sources[].url`에 기재.
4. **수치 미확인 시 추정 금지** — JSON `null` + `note: "본문 미확인"`으로 표기.
5. **검색 시 절대 날짜 명시** — 단순 "NASDAQ today"가 아닌 "NASDAQ close 2026-05-08", "Fed FOMC May 2026 statement" 등. 검색 엔진이 캐시된 옛 결과를 줄 수 있음.
6. **검색 결과의 published date 검증** — 1차 출처 URL의 게시 시각이 작업 시간창(직전 24시간 KST)과 일치하는지 확인. 1년 전 기사가 검색 결과에 섞여 들어올 수 있음. 시간창 밖 기사는 수집 대상 아님.
